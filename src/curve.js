...
function scrubPubKeyFormat(pubKey) {
  ...
  if (pubKey.byteLength === 33) {
    ...
    console.warn("WARNING: Expected pubkey of length 33, please report the ST and client that generated the pubkey");
    return pubKey;
  }
}
...
exports.generateKeyPair = function () {
  if (typeof nodeCrypto.generateKeyPairSync === 'function') {
    ...
    const pubKey = Buffer.from(publicDerBytes.slice(PUBLIC_KEY_DER_PREFIX.length - 1, PUBLIC_KEY_DER_PREFIX.length + 32));
    pubKey[0] = 5;
...
exports.getPublicFromPrivateKey = function (privKey) {
  ...
  const keyPair = curveJs.generateKeyPair(privKey);
